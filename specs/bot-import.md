...
a paste-ready `schedules:` block and says that nothing runs until `aster cron install`. `botmigrate` reaches the same conclusion from the other side: its
...
`bot.json` holds `origin` and a content hash per field. `aster bots update <name>` re-reads the source and shows a three-way view: what the publisher
...
The Telegram surface gets the same entry point: "import this bot: " runs
...
`add` dispatches on what it is handed, the way `aster plugins add` and `aster skills add` already accept a path, a repo, or a URL. `aster bots list` prints
what a bot contributes (agent, skills, routines) for the same reason `aster plugins list` does.

`import` stays a verb under its existing nouns (`aster mcp import`, `aster sessions import`). It is not promoted to a top-level command, because the thing
