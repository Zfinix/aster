...
//! each platform. The chat agent's `search_files` and `list_files` delegate
//! here so they get `rg` / `fd` when present and fall back to embedded or
//! hand-rolled implementations when they are not.
//!
