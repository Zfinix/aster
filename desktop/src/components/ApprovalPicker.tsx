...
import { BoltIcon, FireIcon, ListOrderedIcon, PencilIcon, ReviewIcon, ShieldIcon } from "./icons";
    ...
    <div className="picker picker-modes" role="dialog" aria-label="Mode">
      ...
      {MODES.map((m) => (
        <button
          ...
          data-selected={m.mode === mode && !reviewing}
        ...
        >
          ...
          {m.mode === mode && !reviewing && <span className="picker-check">✓</span>}
        </button>
      ))}
      <button
      ...
      >
        ...
        {reviewing && <span className="picker-check">✓</span>}
      </button>
    </div>
  );
}
